...
void Webserver::webconfig() {
  ...
  webServer.on("/get", HTTP_GET, [] (AsyncWebServerRequest *request) {
      ...
      if (request->hasParam("z21addr2")) {
          z21addr = request->getParam("z21addr2")->value();
          Serial.printf("z21addr2: %s\n", z21addr.c_str());
          Pref::set(prefNameZ21IPAddr2, z21addr);
      } 
...
//  WiFi.mode(WIFI_MODE_APSTA); // AP = Access Point, STA = Station Mode
  ...
  WiFi.mode(WIFI_MODE_STA); // STA = Station Mode
      ...
      M5.lcd.drawString("Verbinde mit WLAN SSID(s):", TFT_W / 2, TFT_H * 0.35, 2);
      M5.lcd.drawString(ssid + (ssid2 == "" ? "" : "  oder "), TFT_W / 2, TFT_H * 0.50, 4); 
      if (ssid2 != "") M5.lcd.drawString(ssid2, TFT_W / 2, TFT_H * 0.65, 4);
      ...
      if (connCount != 0) {
          ...
          M5.lcd.drawString("Netzadressen pruefen!", TFT_W / 2, TFT_H * 0.90, 2);
